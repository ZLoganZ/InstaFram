...
type ImageOptions = 'avatar' | 'post' | 'miniAvatar' | 'story' | 'storyAvatar' | 'default';

export const getImageURL = (image?: string, option: ImageOptions = 'default') => {
  if (!image) return;

  if (image.includes('http')) return image;

  const cld = new Cloudinary({ cloud: { cloudName: import.meta.env.VITE_CLOUDINARY_CLOUD_NAME as string } });
