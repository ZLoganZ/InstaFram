...
export const uploadImage = async (file: File, folder = '') => {
  ...
  formData.append('upload_preset', import.meta.env.VITE_CLOUDINARY_PRESET as string);
...
export const removeImage = async (imageURL?: string) => {
  ...
  const signature = await sha1(
    `public_id=${public_id}&timestamp=${timestamp}${import.meta.env.VITE_CLOUDINARY_API_SECRET}`
  );
  formData.append('signature', signature);
  const res = await fetch(
    `https://api.cloudinary.com/v1_1/${import.meta.env.CLOUDINARY_CLOUD_NAME}/image/destroy`,
    {
      method: 'POST',
      body: formData
    }
